...
//                        . "IdRol INT NOT NULL DEFAULT '1', ";  /* El rol 1 = sin grupo */
    ...
    function CreateTablePermissions($DataBAse)
    {
        ...
        if($estado!=1){echo mysql_error(); return;}
...
//        mysql_close($conexion);
    ...
    function insertar_usuario($detalle_usuario)
    {
        ...
        foreach ($detalle_usuario as $Usuario)
        {
            ...
            if(!file_exists("../Configuracion/$DataBaseName.ini")){echo "<p>No existe el archivo de configuración estructural.</p>"; return;}
...
//                $ClaveEmpresa=filter_input(INPUT_POST, "ClaveEmpresa"); 
//            
//            if(!$DataBaseName>0 or !$ClaveEmpresa>0)
//                return "Falta clave o empresa Nombre de BD.";
//            
//            if(isset($NombreRepositorio['NombreRepositorio']))
//                $NombreRepositorio=$Repositorio['NombreRepositorio'];            
//            else
//                $NombreRepositorio=filter_input(INPUT_POST, "NombreRepositorio");
//            
//            if(!$DataBaseName>0 or !$ClaveEmpresa>0 or !$NombreRepositorio>0)
//                return "Falta clave empresa, Nombre de BD o Nombrel del Repositorio";
//            
...
//                
//                $RepositoryFields_ = $this->GetTableFields($DataBaseName, $NombreRepositorio);
//
...
//                    . ") ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8";
//
//                if(($ResultCrearCatalogo = $this->crear_tabla($DataBaseName,$TablaCatalogo))==1)
//                {
//                    $InsertCatalog = "INSERT INTO Catalogos (IdRepositorio, NombreCatalogo) VALUES "
//                            . "($id)";
//                    
//                    $this->WriteConfigCatalogo("Catalogo_$NombreCatalogo",$configStructure);
//
...
//
//                    if(($ResultAlterTable = $this->ConsultaQuery($DataBaseName, $AlterTable))==1)
//                        echo "<p>Relaciones del catálogo <b>$NombreCatalogo</b> construidas.</p>";
//                    else
//                    {
//                        $DeleteCatalog = "DROP TABLE $NombreCatalogo";
//                        $this->ConsultaQuery($DataBaseName, $DeleteCatalog);
            ...
            if(!$select)
                {
                    ...
                    $ResultadoConsulta = mysql_fetch_row ($select);
...
//                    }
    ...
    public static function QuerySelection($DataBaseName, $Query)
    {
        $estado=true;
        $conexion=  $this->Conexion();
        if (!$conexion) {
     ...
     * Realiza una consulta especifícando la instancia de BD y el query a ejecutar.
     */
    public function Query($DataBasaName, $query)
    {
        $estado=true;
        $conexion=  $this->Conexion();
        if (!$conexion) {
